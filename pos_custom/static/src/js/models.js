function openerp_pos_models(instance, module){ //module is instance.point_of_sale
    ...
    module.PosModel = Backbone.Model.extend({
        ...
        models: [
        {
            ...
            model:  'res.partner',
            fields: ['name','street','city','state_id','country_id','vat','phone','zip','mobile','email','ean13','write_date', 'credit', 'customer'],
        ...
        load_new_partners: function(){
            ...
            new instance.web.Model('res.partner')
                .query(fields)
                //.filter([['write_date','>',this.db.get_partner_write_date()]])PCAROLI: Just for testing
                .filter([['customer', '=', true]])//PCAROLI: only use customers
                .all({'timeout':3000, 'shadow': true})
                .then(function(partners){
                    if (self.db.add_partners(partners)) {   // check if the partners we got were real updates
                        def.resolve();
                    } else {
                        def.resolve();//PCAROLI: Was def.reject();
                    }
